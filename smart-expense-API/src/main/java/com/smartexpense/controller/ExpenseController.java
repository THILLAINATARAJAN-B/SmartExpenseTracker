...
public class ExpenseController {
    ...
    public ResponseEntity<List<ExpenseDTO>> filter(
        ...
        if (category != null && from != null && to != null) {
            return ResponseEntity.ok(expenseService.filterByCategoryAndDate(userId, category, from, to));
        } else if (category != null) {
            return ResponseEntity.ok(expenseService.filterByCategory(userId, category));
        } else if (from != null && to != null) {
            return ResponseEntity.ok(expenseService.filterByDateRange(userId, from, to));
        }
        return ResponseEntity.ok(expenseService.getExpensesByUser(userId));
    }
